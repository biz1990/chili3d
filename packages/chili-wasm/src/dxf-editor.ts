...
export class DxfEditor {
    ...
    move(shape: IShape, vector: XYZ): IShape {
        if (shape.shapeType === "edge" || shape.shapeType === "wire") {
            ...
            transformation.SetTranslationPart(vector.x, vector.y, vector.z);
    ...
    scale(shape: IShape, factor: XYZ, center?: XYZ): IShape {
        if (shape.shapeType === "edge" || shape.shapeType === "wire") {
            ...
            const scaleCenter = center || { x: 0, y: 0, z: 0 };
...
export class DxfToSolidWorkflow {
    ...
    wireToSketch(wire: IShape): IShape {
        // For now, just return the wire as is
    ...
    extrude(shape: IShape, direction: XYZ, distance: number): Result<IShape> {
        if (shape.shapeType === "wire" || shape.shapeType === "face") {
            const occShape = (shape as any).shape;
            const dir = new wasm.gp_Dir(direction.x, direction.y, direction.z);
            
            const extruded = wasm.ShapeFactory.prism(occShape, new wasm.gp_Vec(
                dir.X() * distance,
                dir.Y() * distance,
                dir.Z() * distance
            ));
        ...
        return Result.err("Cannot extrude shape type: " + shape.shapeType);
    }
    ...
    revolve(shape: IShape, axis: { point: XYZ; direction: XYZ }, angleDegrees: number): Result<IShape> {
        if (shape.shapeType === "wire" || shape.shapeType === "edge") {
            ...
            const revolved = wasm.ShapeFactory.revolve(occShape, axis, angleDegrees);
        ...
        return Result.err("Cannot revolve shape type: " + shape.shapeType);
    }
